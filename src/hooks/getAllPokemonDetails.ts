      ...
      try {
        const detailsPokemon = await Promise.all(
          pokemonList.map(async (pokemon) => {
            ...
            const storedPokemon = localStorage.getItem(pokemon.name);
            if (storedPokemon) {
            ...
            const pokemonData = {
              id: data.id,
              name: data.name,
              health: data.stats.find((stat: any) => stat.stat.name === "hp")
                ?.base_stat,
            ...
            localStorage.setItem(pokemon.name, JSON.stringify(pokemonData));
